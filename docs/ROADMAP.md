...
| M3.2.6a/b/c | Signal positions and queue counters. **a** (D64): A21 evidence, before/on/after a cut, stretch/split/copy, both sides — **implemented 2026-09-25**. **b** (D64): `AuthoredQueueCounter`, schema 16, place-based evaluation, one row per approach — **implemented 2026-09-25**. **c**: counter tool, Queue counters table, en/th, save/reopen | a/b: `signal_position.*`, `queue_counter.*`; c open |
