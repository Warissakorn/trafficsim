...
| M3.2.8a/b | Behaviour ([M3_8_CONTRACT.md](M3_8_CONTRACT.md)). **a** (D69): commitment at a waiting line — a driver who cannot stop at `maxDeceleration` ignores headway/gap time, never occupancy, Stop, receiving space or the swept check; zones and derived merges — **implemented 2026-09-26**. **b**: lane changing, cooperation, visibility and remaining behavior — open, contract §2 unwritten | a: `commitment.*`, `docs/evidence/m3.2.8a-*`; b: controlled changes/conflicts, congestion and replay; calibrated gap acceptance still requires M6 evidence |
