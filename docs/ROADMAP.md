...
| M3.2.7a/b/c | T-junction evidence (A26). **a** (D66): fixture through commands (both sides, Yield/Stop, blocked exit) and controlled gap/headway/movement cases — **implemented 2026-09-25**. **b** (D66): diagnostic seeded sweep, metadata committed first — **implemented 2026-09-25**. **c**: signal-composition variant and the owner exercise (§3, Windows) | a: `tjunction.*`, `tjunction_controlled.*`; b: `docs/evidence/m3.2.7-sweep.*`; c open |
| M3.2.8 | Lane changing, cooperation, visibility and remaining behavior, **including a commitment rule at a waiting line** (M3.2.7a found minor vehicles clamped when the gap closes as they reach the line, D66) | Open; separate contract, controlled changes/conflicts, congestion and replay; calibrated gap acceptance still requires M6 evidence |
