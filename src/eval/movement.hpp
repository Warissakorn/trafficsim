...
// One approach: the signal heads (one per lane) whose stop lines the queue is measured from.
struct QueueCounter { std::string name; std::vector<std::string> headIds; };
...
private:
    ...
    std::vector<std::size_t> counterOfHead_;   // parallel to Scenario::signalHeads; npos = none
