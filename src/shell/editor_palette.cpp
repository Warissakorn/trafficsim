...
void EditorWindow::buildPalette() {
    ...
    palette_=new QListWidget(body);palette_->setObjectName("editorObjectPalette");layout->addWidget(palette_);
    const int modes[]={0,1,5,6,7,8,2,3,4};
    const char* shortcuts[]={"S","L","C","R","V","H","X","M","K"};
    for(int row=0;row<9;++row){
        auto* entry=new QListWidgetItem(palette_);entry->setData(Qt::UserRole,modes[row]);
    ...
    auto* help=new QLabel(body);help->setWordWrap(true);texts_["editorGestureHelp"]=help;layout->addWidget(help);
    ...
    auto* button=new QToolButton(body);button->setDefaultAction(background);layout->addWidget(button);
