...
- **Gates:** M1 usability accepted by owner ruling (D49; `docs/M1_ACCEPTANCE.md` keeps what the
  ...
  **M0 plausibility is still open.** M3.2.2a–c, M3.2.3a–c, M3.2.4a–b, M3.2.5a–b, M3.2.6a–c and M3.2.7a–b are done (D54–D66): authored crossings and merges run on
  the admission solver with Stop/Yield, all authorable in the editor; queue counters are place-based and authorable; the T-junction evidence (fixture, controlled cases, sweep) is in; next is **M3.2.7c** (signal variant, congested headway case, owner exercise) — `docs/NEXT.md`.
